...
- `**WorkScheduler**`: thin WorkManager wrapper that will schedule recurring generation, reminders, auto-backup, and budget evaluation.
...
- `**NotificationHelper**`: will own notification channels for reminders, summaries, and budget alerts.
...
| `ReminderService` / reminder & summary workers          | Domain/Work | P4      | §5.11   |
...
|                    |                                                                           |
| ------------------ | ------------------------------------------------------------------------- |
| **Responsibility** | Single activity host; edge-to-edge; `NavHostFragment` + bottom navigation |
| **Depends on**     | Navigation graph, theme                                                   |
| **Phase**          | P1                                                                        |
...
|                    |                                                                                                                                                                                 |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Responsibility** | Currency, default payment method, categories, about (P1); theme, app lock, backup, export/import (P2); templates, recurring (P3); reminders (P4); budgets, goals (P5)           |
...
### 5.11 ReminderService (P4)

Reminders and summaries will be scheduled work that respects notification permission.

- **Daily reminder**: configurable time → `ReminderWorker` → notification.
- **Summaries**: weekly/monthly `SummaryWorker` will pull totals from `AggregationService`.
- **Depends on**: `WorkScheduler`, `NotificationHelper`, `AppPreferences`.
...
3. `ReminderService` + reminder/summary workers
...
- [ ] P4: Live import queue; reminders & summaries; notes page
...
- **Store/key password:** `427c4444cdcd6ef3e1f4a4a51d0edd72f67b4df6`

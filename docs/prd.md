...
| **P4** | Live import & reminders | SMS/notification/clipboard review queue; daily logging reminder shipped; summary notifications later; notes page | Later  |
...
| F-25 | Reminders & summaries    | P4    | Daily log reminder shipped; weekly/monthly summary notifications remain future work |
...
| ID    | Requirement                                                           |
| ----- | -------------------------------------------------------------------- |
| FR-68 | User receives a **daily reminder** to log expenses at the configured reminder time. |
| FR-69 | The app sends the reminder only when **no expenses were logged today**. |
| FR-70 | Tapping the reminder opens the **expense logger** directly.          |
| FR-71 | Reminders require notification permission on Android 13+; if denied, reminders are suppressed. |

Weekly/monthly summary notifications remain planned for a later reminder release.
...
- [ ] Daily reminder fires when notifications are allowed, skips days with logged expenses, and opens the logger.
- [ ] Weekly/monthly summary notifications remain pending.
